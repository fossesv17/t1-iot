...
def headerToDict(data):
    ...
    dict = {"ID_device": ID_Device.decode().rstrip("\x00"), "MAC": MAC.decode(), "Transport_layer": transport_layer,
...
def dictToHeader(dict):
    ...
    paquete = pack("<8s17s3i", id_device.encode(), mac.encode(), trans_layer, protocol, length)
    return paquete
...
def dictToData(headers, dict):
    ...
    if id_protocol == 0:
        ...
        rest_pack = pack("<H19s4i", batt_lvl, timestp, temp, press, hum, co)
...
def dataToDict(protocol, data):
    ...
    if protocol == 0:
        ...
        data1 = [batt_lvl, timestmp.decode()]
        ...
        batt_lvl, timestmp, temp, press, hum, co = unpack("<H19siiii", data)
