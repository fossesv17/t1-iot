...
void protocol4(struct proto4 *pPack, uint16_t msg_id, uint8_t *mac, uint8_t transport_layer) {
    ...
    pPack->hum = thpc_sens_hum();
    pPack->co = thpc_sens_co();
    memcpy(pPack->accx, accel_sens_accx(), 2000 * sizeof(float));
    memcpy(pPack->accy, accel_sens_accy(), 2000 * sizeof(float));
    memcpy(pPack->accz, accel_sens_accz(), 2000 * sizeof(float));
    memcpy(pPack->rgyrx, accel_sens_rgyr(), 2000 * sizeof(float));
    memcpy(pPack->rgyry, accel_sens_rgyr(), 2000 * sizeof(float));  
    memcpy(pPack->rgyrz, accel_sens_rgyr(), 2000 * sizeof(float));
}

void create_packet(char *buf, uint16_t msg_id, uint8_t *mac, uint8_t transport_layer, uint8_t protocol) {
    if (protocol == 0) {
        struct proto0 p0;
